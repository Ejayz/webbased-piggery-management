...
export default function QrCode({ setter, index, setHide, hide }: any) {
  ...
  let html5QrCode: any;
  useEffect(() => {
    if (html5QrCode !== undefined && hide.show == false) {
      html5QrCode
        .stop()
        .then((ignore: any) => {
      ...
      html5QrCode
        .start(
          ...
          (decodedText: any, decodedResult: any) => {
            console.log(index, decodedText);
            setter(index, decodedText);
            setHide({ show: false, index: 0 });
          },
          (errorMessage: any) => {}
        )
        .catch((err: any) => {});
