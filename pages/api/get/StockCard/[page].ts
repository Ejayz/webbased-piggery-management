...
async function GetUsersWithSearch(
  ...
  try {
    keyword = `%${keyword}%`;
    const sql = `SELECT *,tbl_inventory.* FROM tbl_stock_card   INNER JOIN tbl_inventory ON tbl_inventory.item_id = tbl_stock_card.item_id WHERE  tbl_stock_card.item_id=? ORDER BY stock_card_id  ASC LIMIT 1 OFFSET ${offset};`;
