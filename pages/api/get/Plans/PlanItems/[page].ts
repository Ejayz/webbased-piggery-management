...
async function SearhGetCage(
  ...
  try {
    keyword = `%${keyword}%`;
    const sql = `SELECT * FROM tbl_objective_item_detail INNER JOIN tbl_inventory ON tbl_inventory.item_id=tbl_objective_item_detail.item_id  WHERE plan_details_objectives=? and tbl_objective_item_detail.is_exist = 'true' AND (item_name LIKE ? OR item_quantity LIKE ?) ORDER BY ${conn.escapeId(
      sortby
    )} ${SortOrder}
    LIMIT ${limit} OFFSET ${offset};`;
    const [err, result] = await conn.query(sql, [plan_id, keyword, keyword]);
    conn.release();
    if (err) return err;
    return result;
  } catch (error) {
