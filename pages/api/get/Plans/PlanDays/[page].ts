...
export default async function handler(
  ...
  const { keyword, sortby, sortorder, plan_id }: any = JSON.parse(filter);
  if (page == "0") {
    return res
      .status(404)
...
async function SearhGetCage(
  ...
  try {
    keyword = `%${keyword}%`;
    const sql = `SELECT *, (SELECT COUNT(*) FROM tbl_plan_details_objectives WHERE tbl_plan_details_objectives.plan_detail_id=tbl_plan_details.plan_detail_id AND tbl_plan_details_objectives.is_exist='true') AS \`total_objectives\`  FROM tbl_plan_details    WHERE plan_id=? and is_exist = 'true' AND (day LIKE ?) ORDER BY ${conn.escapeId(
      sortby
    )} ${SortOrder}
    LIMIT ${limit} OFFSET ${offset};`;
    const [err, result] = await conn.query(sql, [plan_id, keyword]);
    conn.release();
    if (err) return err;
    return result;
  } catch (error) {
