...
async function UpdateCage(batch_id: any) {
  ...
  try {
    const sql =
      "SELECT * FROM tbl_operation INNER JOIN tbl_cage ON tbl_operation.cage_id=tbl_cage.cage_id  INNER JOIN tbl_operation_type ON tbl_operation_type.operation_type_id=tbl_operation.operation_type_id WHERE tbl_operation.is_exist='true' and tbl_operation.operation_type_id=1 and tbl_operation.cage_id=? order by tbl_operation.operation_date asc";
