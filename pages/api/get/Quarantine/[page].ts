...
async function GetCage(
  ...
  try {
    const sql = `SELECT *
    FROM tbl_batch LEFT JOIN tbl_pig ON tbl_batch.pig_id = tbl_pig.pig_id
   WHERE (batch_name LIKE ?) AND tbl_pig_historu.is_exist='true' 
...
async function SearhGetCage(
  ...
  try {
    keyword = `%${keyword}%`;
    const sql = `SELECT * FROM tbl_pig_history INNER JOIN tbl_cage ON tbl_cage.cage_id=tbl_pig_history.cage_id WHERE pig_status='Quarantined' AND tbl_pig_history.is_exist='true' and
   (tbl_cage.cage_name like ? OR tbl_pig_history.pig_id like ? and tbl_pig_history.remarks like ? )  
     ORDER BY ${conn.escapeId(
       sortby
     )} ${sortorder}  LIMIT ${limit} OFFSET ${offset} ;`;
