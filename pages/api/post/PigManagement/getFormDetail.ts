...
async function Ops() {
  ...
  try {
    ...
    const getBoarList =
      "select * from tbl_pig INNER JOIN tbl_pig_history ON tbl_pig_history.pig_id=tbl_pig.pig_id  where pig_type='Boar' and tbl_pig.is_exist='true' and tbl_pig_history.pig_history_status='active'";
    const [BoarList] = await conn.query(getBoarList);
    const getSowList =
      "select * from tbl_pig INNER JOIN tbl_pig_history ON tbl_pig_history.pig_id=tbl_pig.pig_id   where pig_type='Sow' and tbl_pig.is_exist='true' and tbl_pig_history.pig_history_status='active' and selectable!='2'";
