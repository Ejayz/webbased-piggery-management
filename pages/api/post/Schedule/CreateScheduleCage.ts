      ...
      item_list.map(async (item: any) => {
        if (item.operation_id == 1) {
          const insertOperation =
            "insert into tbl_operation (operation_type_id,operation_date,cage_id,am_pm) values (?,?,?,'AM') ";
          ...
          const insertOperationPM =
            "insert into tbl_operation (operation_type_id,operation_date,cage_id,am_pm) values (?,?,?,'PM') ";
          ...
          const insertOperation =
            "insert into tbl_operation (operation_type_id,operation_date,cage_id) values (?,?,?) ";
          const [sqlInsertResult]: any = await conn.query(insertOperation, [
            item.operation_id,
            item.operation_date,
            pig_id,
